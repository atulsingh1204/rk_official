...
public class JodiFamilyActivity extends AppCompatActivity implements View.OnClickListener, ItemClickListener {
   ImageView iv_back;
         ...
         public void onResponse(Call<GetSingleDigitResponse> call, Response<GetSingleDigitResponse> response) {
            ...
            if (response.body() != null) {
               if (response.body().getStatus().equals("true")) {
                  ...
                  Toast.makeText(JodiFamilyActivity.this, response.body().getMessage(), Toast.LENGTH_SHORT).show();
               }
            }
         }
         ...
         case R.id.bt_add:
            ...
            if (visibility_open) {
               if (tv_date.getText().toString().trim().equals("SELECT DATE")) {
                  ...
                  actv_digit.setError("Jodi Digit Required ! !");
                  ...
                  actv_digit.setError("Invalid Jodi Digit !");
                  actv_digit.requestFocus();
   ...
   private void insertGame(String date, String digit, String point) {
      ...
      for (int i3 = 0; i3 < 8; i3++) {
         String valueOf = String.valueOf(digit.charAt(0));
         String valueOf2 = String.valueOf(digit.charAt(1));
         if (i3 == 0) {
            List<FamilyNumberModel> list = familyNumberModelList;
            list.add(new FamilyNumberModel(valueOf + valueOf2, "f"));
         } else if (i3 == 1) {
            if (!valueOf.equals(valueOf2)) {
               List<FamilyNumberModel> list2 = familyNumberModelList;
               list2.add(new FamilyNumberModel(valueOf2 + valueOf, "f"));
            }
         } else if (i3 == 2) {
            if (!valueOf.equals(valueOf2)) {
               List<FamilyNumberModel> list3 = familyNumberModelList;
               list3.add(new FamilyNumberModel(valueOf2 + jodiList.get(Integer.parseInt(valueOf)), "f"));
            } else {
               List<FamilyNumberModel> list4 = familyNumberModelList;
               list4.add(new FamilyNumberModel(jodiList.get(Integer.parseInt(valueOf)) + valueOf2, "f"));
            }
         } else if (i3 == 3) {
            if (!valueOf.equals(valueOf2)) {
               List<FamilyNumberModel> list5 = familyNumberModelList;
               list5.add(new FamilyNumberModel(jodiList.get(Integer.parseInt(valueOf)) + valueOf2, "f"));
            }
         } else if (i3 == 4) {
            if (!valueOf.equals(valueOf2)) {
               List<FamilyNumberModel> list6 = familyNumberModelList;
               list6.add(new FamilyNumberModel(jodiList.get(Integer.parseInt(valueOf2)) + valueOf, "f"));
            }
         } else if (i3 == 5) {
            List<FamilyNumberModel> list7 = familyNumberModelList;
            list7.add(new FamilyNumberModel(valueOf + jodiList.get(Integer.parseInt(valueOf2)), "f"));
         } else if (i3 == 6) {
            List<FamilyNumberModel> list8 = familyNumberModelList;
            list8.add(new FamilyNumberModel(jodiList.get(Integer.parseInt(valueOf)) + jodiList.get(Integer.parseInt(valueOf2)), "f"));
         } else if (i3 == 7 && !valueOf.equals(valueOf2)) {
            List<FamilyNumberModel> list9 = familyNumberModelList;
            list9.add(new FamilyNumberModel(jodiList.get(Integer.parseInt(valueOf2)) + jodiList.get(Integer.parseInt(valueOf)), "f"));
         }
      }
      while (i2 < familyNumberModelList.size()) {
         int i4 = i2 + 1;
         for (int i5 = i4; i5 < familyNumberModelList.size(); i5++) {
            if (familyNumberModelList.get(i2).getNumber().equals(familyNumberModelList.get(i5).getNumber())) {
               familyNumberModelList.remove(i5);
            }
         }
         i2 = i4;
      }
      
      addDataInCart(date, point);
   }
   
   private void addDataInCart(String date, String point) {
      if (cartModelList.size() > 0) {
         customDialog.showFailureDialog("Please Submit Game before playing again");
      } else {
         if (checkBalance(String.valueOf(familyNumberModelList.size() * Integer.parseInt(point)))) {
            String newGame = "Jodi";
            for (int i2 = 0; i2 < familyNumberModelList.size(); i2++) {
               String current_time = new SimpleDateFormat("HH:mm:ss", Locale.getDefault()).format(new Date());
               CartModel cartModel = new CartModel(userId, company_id, game_id, newGame, "2", point, familyNumberModelList.get(i2).getNumber(), changeToServerDate(date), current_time);
      ...
      sweetAlertDialog.setTitleText(msg)
            ...
            .setConfirmClickListener(new SweetAlertDialog.OnSweetClickListener() {
               ...
               public void onClick(SweetAlertDialog sDialog) {
                  ...
                  Intent i = new Intent(JodiFamilyActivity.this, MainActivity.class);
