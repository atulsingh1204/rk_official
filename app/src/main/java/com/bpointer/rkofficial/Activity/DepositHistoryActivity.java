...
import com.bpointer.rkofficial.Adapter.DepositHistoryAdapter;
...
public class DepositHistoryActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void getDepositHistoryAPI(String date) {
        ...
        Call<DepositHistoryResponseModel> call = Api.getClient().create(Authentication.class).getDepositHistory(requestBody);
        call.enqueue(new Callback<DepositHistoryResponseModel>() {
            @Override
            public void onResponse(Call<DepositHistoryResponseModel> call, Response<DepositHistoryResponseModel> response) {
                customDialog.closeLoader();
                if (response.body() != null) {
                    if (response.body().getStatus().equals("true")) {

                        DepositHistoryAdapter depositHistoryAdapter = new DepositHistoryAdapter(DepositHistoryActivity.this, response.body().getDepositHistory());
                        rv_history.setAdapter(depositHistoryAdapter);
                    } else {
                        Toast.makeText(DepositHistoryActivity.this, response.body().getMessage(), Toast.LENGTH_SHORT).show();
                        DepositHistoryAdapter depositHistoryAdapter = new DepositHistoryAdapter(DepositHistoryActivity.this, response.body().getDepositHistory());
                        rv_history.setAdapter(depositHistoryAdapter);
                    }
                }
            ...
            @Override
            public void onFailure(Call<DepositHistoryResponseModel> call, Throwable t) {
                customDialog.closeLoader();
    ...
    private void initView() {
        ...
        tv_title.setText("Deposit History");
