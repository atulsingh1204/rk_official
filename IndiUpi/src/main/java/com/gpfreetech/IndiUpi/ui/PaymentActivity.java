...
public final class PaymentActivity extends AppCompatActivity {
    ...
    void removePhonepe(List<ResolveInfo> list) {
        for (Iterator<ResolveInfo> iterator = list.iterator(); iterator.hasNext();) {
            ResolveInfo number = iterator.next();
            if ("com.phonepe.app".equalsIgnoreCase(number.activityInfo.packageName)) {
                iterator.remove();
                break;
            }
        }
